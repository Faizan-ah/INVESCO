...
const useStyles = makeStyles((theme) => ({
 ...
 function SimpleTabs(props) {
  ...
  const [houseVars,setHouseVars] = React.useState({size:'Marla',price:'0'})
  ...
  const validate = ()=>{
    var errorMsg;
    console.log('in')
    if(houseVars.area=='e' || houseVars.area==''){
      console.log('ineee')
      errorMsg = setHouseVars({...houseVars,error:'Enter Valid Area'})
      return false
    }
    else if(houseVars.bed=='e' || houseVars.bed==''){
      errorMsg = setHouseVars({...houseVars,error:'Enter Your Bedroom(s)'})
      return false
    }
    else if(houseVars.bath=='e' || houseVars.bath==''){
      errorMsg = setHouseVars({...houseVars,error:'Enter Your Bathroom(s)'})
      return false
    }
    return true
  }
  const calculatePrice = ()=>{
    setLoading(true)
    fetch(`https://housingpredict.herokuapp.com/prediction/?size=${houseVars.area}%20${houseVars.size}&&baths=${houseVars.bath}&&beds=${houseVars.bed}&&location=${houseVars.location}`)
  .then((response)=>{
      response.json().then((responseData)=>{
       ...
       setHouseVars({...houseVars,price:responseData,error:''})
        ...
        if(houseVars.area=='e' || houseVars.area=='' || negativeNum.test(houseVars.area) || houseVars.area =='0' ){
          setHouseVars({...houseVars,price:'0',error:'Enter Valid Area',area:''})
        }
        else if(houseVars.bath=='e' || houseVars.bath==''|| !negativeNum.test(houseVars.bath)){
          setHouseVars({...houseVars,price:'0',error:'Enter Your Bathroom(s)',bath:''})
        }
        else if(houseVars.bed=='e' || houseVars.bed==''|| !negativeNum.test(houseVars.bed)){
          setHouseVars({...houseVars,price:'0',error:'Enter Your Bedroom(s)',bed:''})
        }
        else if(houseVars.location!=top100Films.map(option => option.title)){
          setHouseVars({...houseVars,price:'0',error:'Enter a Valid Location',location:''})
        }
        else{
  ...
  console.log('abhi ')
  console.log('area',houseVars.area, 'size',houseVars.size, 'bed',houseVars.bed,'bath',houseVars.bath,'location',houseVars.location)
  console.log('isAutttata ',isAuthenticated.isAuth)
  if(isAuthenticated.isAuth==false){
    console.log('GATDGASDGAGSDGASGD')
    return(
        <Login/>
    )
}else{
  console.log('NOT FOING')
  return (
        ...
        <div className='house-menu'>
              ...
              <div className='house-menu-inputs-rows'>
                {/* AREA AND SEARCH BAR */}
                  <div className='house-menu-inputs-row-1'>
                    <TextField size='large' id="standard-basic"  type='number' name='area' value={houseVars.area} onChange={onChangeInputs} variant="outlined" label="Area Size" />
                    <div>
                      <select onChange={(e)=>{setHouseVars({...houseVars,size: e.target.value})}}name="size" id="size">
                      ...
                      <Autocomplete
                        ...
                        id="free-solo-demo"
                        onChange={(event,value)=>{setHouseVars({...houseVars,location:value})}}
                        freeSolo
                        options={top100Films.map((option) => option.title)}
                        renderInput={(params) => (
                          <TextField {...params} name='location' value={houseVars.location}  onClick={onChangeInputs} label="Location" margin="normal" variant="outlined" />
                        )}
                      />
              </div>
              {/* BATH */}
                <TextField type='number' size='large' min="0" oninput="validity.valid||(value='');" name='bath' value={houseVars.bath} onChange={onChangeInputs} id="standard-basic" variant="outlined" label="Bath(s)" />
              {/* BEDS */}
                <TextField type='number' size='large' name='bed' value={houseVars.bed} onChange={onChangeInputs} id="standard-basic" variant="outlined" label="Bed(s)" />
          ...
          <div className='house-menu-right'>
            ...
            <div className='calculated-price'>
              Calculated Price : 
              {isLoading == false ? <span> {houseVars.price} PKR</span>: 
              <CircularProgress size='20px' style={{marginLeft:'80px'}} disableShrink /> 
              }
          ...
          <div style={{marginTop:'20px'}}>
            <span style={{color:'red',fontWeight:'bold',fontSize:'18px'}}>{houseVars.error}</span>
          </div>
        </div>
      </TabPanel>
